                            ...
                            <div class="large-12 medium-12  columns insert_in">
                                ...
                                <select class="group_id col-md-6">
                                    <?foreach($groups as $gr){?>
                                        <option value="<?=$gr['id'];?>" <? if($_GET['group_id'] == $gr['id']){?>selected="selected" <?}?>><?=$gr['name'];?></option>
                                    <?}?>
                                </select>
...
$('.route').change(function(){
    ...
    $.ajax({
        type: "POST",
        url: "/ferryman/ajax_get_price_table_edit_return",
        data: {route_name_id:$('.route').val(),valute:$('.valute').val(),fer_id:$('.ferryman').val(),group_id:$('.group_id').val()},
        async: false,
...
$('.update_discount_price').click(function(){   
   ...
   $.ajax({
        ...
        success: function(data) {
            //alert(data);
            //Sp = data 
            location="/groups/group_price_return";           
        },
        error:function(){
