    ...
    <div class="modal-content tab-content" >
      ...
      <div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
              <div class="modal-dialog modal-lg"  >
                ...
                <div class="modal-content tab-content" style="background-color:#f1fae2 !important">
                    <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <br />
                    </div>
                    ...
                    <div class="row form-row">
                <div class="col-md-12 m-b-20">
        <h3 class="bold"><?=__('To book')?></h3>
        </div>
     
                    <div class="col-md-12">
                    <button id="order_tik" type="button" class="btn btn-success btn-cons" data-dismiss="modal"><?=__("Confirm booking")?></button>
                       <button type="button" class="btn btn-default btn-cons" data-dismiss="modal"><?=__("Cancel booking")?></button>
                    </div>
...
$("#order_tik").click(function(){    
    ...
    if(return_inputs.length != 0){    
        ...
        var return_plase = [];
        $('.return_people .one_people .plase').each(function(){
       ...
       $('.return_people .one_people .main_discount_id').each(function(){
        ...
        $.ajax({
            ...
            url: "/tiket/ajax_sell_tikets",
            async:false,
            data: {route_name_id:return_Rout,from:return_From,to:return_To,plases:return_plase,inputs:return_inputs,price:return_price,valute:return_Valute,buses_id:return_Bus,ferryman_id:return_Fer,route_date:return_RouteDate,lang:$("#cur_lan").val(),du:2,sesuser:$('.sesuser').val(),discount:return_dis,rice:Rice_rerurn,vedomost:0,route_date_newday:RouteDate_newDay_return,route_date_end:RouteDate_end_return},
            success: function(data) { 
                //alert(data);
                //exit;
       ...
       $.ajax({
        ...
        async: false,
        data: {tik_arrays:tik_arrays,lang:$("#cur_lan").val(),valute:$('.sell_vall').val(),full_price:$('.al_bascet').html()},
        success: function(data) {
            //Print.push(data);
    ...
    var stroka = ':';
    tik_arrays.forEach(function(item, i, tik_arrays) {
       stroka += item+"";
       });
    modal_info_open('<h3><?=__("Tickets are booked")?></h3><h5>'+stroka+'</h5>');
...
$("#sell_tik").click(function(){
    ...
    $.ajax({
        ...
        async: false,
        data: {tik_arrays:tik_arrays,tik_arrays_open:tik_arrays_open,lang:$("#cur_lan").val(),valute:$('.sell_vall').val(),full_price:$('.bottom_block_sell .al_bascet').html()},
        success: function(data) {
            //Print.push(data);
            //alert(data);
            //exit;
