<style>
.ajax {
    position: absolute;
    background-color: #e5e5e5;
    
}
.ajax ul{
    padding: 10px;
}
.ajax ul li{
    cursor: pointer;
}
</style>
<?
    $lang = Session::instance()->get('leng');
    if(!$lang) {
        $lang = 'EN';
    } 
    
    //echo '<pre>';
    //print_r($keys);
    //echo '</pre>';
    
    
?>
<div class="content">  
	<div class="row" id="inbox-wrapper">
    <div class="col-md-12">
       <div class=""><h2 class=" inline"><?=__("Join Routes Osts")?> - <?=$join_route;?></h2></div>
    </div>
    </div>
</div>
<div class="content"> 
		<div class="row" id="inbox-wrapper">
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-12">
						 <div class="">
							<div class="grid-body no-border email-body" style="min-height: 850px;">
    							     <br/>
    							 <div class="row-fluid">
    							 <div class="row-fluid dataTables_wrapper">
    									<div class="clearfix"></div>
							     </div>
    							    <div class="col-md-12 no-padding">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th><? echo __('Id');?></th>
                                                <th><? echo __('Station');?></th>
                                                <th><? echo __('Join ost ID');?></th>
                                            </tr>
                                        </thead>
                                        <tbody class="so">           
                                            <? foreach($all_ost as $ost){?>              
                                                <tr>        
                                                    <td><span><?=$ost['route_city_id'];?></span></td>                    
                                                    <td><? $ost_name = DB::select()->from('routecity_i18n')->where('route_city_id','=',$ost['route_city_id'])->and_where('culture','=',$lang)->execute()->current(); echo $ost_name['name_i18n'].' - '; echo $ost_name['city_i18n'];?></td>
                                                    <td>
                                                        <? $chek_join_ost = DB::select()->from('regabus_join_osts')->where('svitgo_ost_id','=',$ost['route_city_id'])->and_where('join_route','=',$join_route)->execute()->current(); if(empty($chek_join_ost)){?>
                                                        <input name="time" type="text" class="t-t-input join_ost" value=""/><button class="btn btn-danger" onclick="save_join_ost('<?=$join_route?>','<?=$ost['route_city_id'];?>',$(this).prev('.join_ost').val())">Join</button>
                                                        <?}else{ echo $chek_join_ost['reg_ost_id']; ?><button class="btn btn-success" onclick="dell_join_ost('<?=$chek_join_ost['id'];?>')">Delete</button><?}?>
                                                    </td>
                                                </tr>
                                            <?}?>    
                                        </tbody>
                                    </table>           
                                </div>	
                            </div>
                        </div>
                     </div>
                </div>
            </div>
       </div>
    </div>
</div>
<script type="text/javascript">
function save_join_ost(join_route,svitgo_ost,join_ost){
    $.ajax({
        type: "POST",
        url: "/sysuser/add_join_route_ost",
        data: {join_route:join_route,svitgo_ost:svitgo_ost,join_ost:join_ost},
        success: function( data ) {
            //alert(data);
            //exit;
            //if(data == 'ok'){
                //location="/adminRoute/regabus_osts";
                
            location="/sysuser/see_join_route?join_id=<?=$_GET['join_id'];?>";
            //}
        },
        error:function(code, opt, err){
            alert("Состояние : " + opt + "\nКод ошибки : " + code.status + "\nОшибка : " + err);
        }
    });
}

function dell_join_ost(ID){
    $.ajax({
        type: "POST",
        url: "/sysuser/dell_join_route_ost",
        data: {id:ID},
        success: function( data ) {
            //alert(data);
            //exit;
            //if(data == 'ok'){
                //location="/adminRoute/regabus_osts";
                
            location="/sysuser/see_join_route?join_id=<?=$_GET['join_id'];?>";
            //}
        },
        error:function(code, opt, err){
            alert("Состояние : " + opt + "\nКод ошибки : " + code.status + "\nОшибка : " + err);
        }
    });
}

function join_rega(regaID){
    
    $('<div style="padding:20px;" class="pr"></div>').dialog({
            modal: true,
			resizable: false,
            open: function () {
                $(this).html('<div class="gh" style="padding-bottom:20px;"></div><div><span class="reg_id">'+regaID+'</span><input name="to" value="<? echo __('Куда');?>" class="inputString_to"  class="route_input_to from-to extrafild"/><input type="text" class="svitgo_id" value="" /><button onclick="save_reg_join('+regaID+',$(this).prev(\'.svitgo_id\').val())">Связать</button><div><div class="clearfix"></div>');
            },
            close: function(event, ui) {
                    $(this).remove();
                    $('.m-r-a-p, .m-t-price').show();
                },
          //  height: 500,
            width: 990,
            title: '<? echo __('Отправить');?>'
        });
}

function save_reg_join(reg_id,svitgo_id) {
    //alert(reg_id);
    //exit;
    $.ajax({
        type: "POST",
        url: "/sysuser/join_reg_ost",
        data: {reg_id:reg_id,svitgo_id:svitgo_id},
        success: function( data ) {
            //alert(data);
            //exit;
            //if(data == 'ok'){
                //location="/adminRoute/regabus_osts";
                
                location="/sysuser/see_join_route?join_id=<?=$_GET['join_id'];?>";
            //}
        },
        error:function(code, opt, err){
            alert("Состояние : " + opt + "\nКод ошибки : " + code.status + "\nОшибка : " + err);
        }
    });
}
$( ".inputString_to" ).keyup(function(){
    var IMP = $(this).parent('td');
    $.ajax({
        type: "POST",
        url: "/sysuser/auto_new_POST",
        data: {val:$(this).val(),lang:"RU"},
        success: function(data) {
            //if(data == 'ok'){
            //    location='/pay/see_bus_schema';
            //}
            //alert(data);
            IMP.find('.ajax').html(data);
        },
        error:function(){
            alert('ошибка bus_step');
        }
    });
})
</script>


