...
<h5><?=Date('d.m.Y',strtotime($date));?> <?=$day_week;?></h5>
    ...
    <div class="selector-heading">
                ...
                <div class="col-md-4 label--details hed"><?=__("Ferryman")?></div>
    ...
    <?foreach($routes as $route){
        ...
        <div class="one_blol_search_route first_people">
            <div class="selector-heading-result left_ser">
                ...
                <div class="col-md-4 label--details cell"><?=$fermann_id['name'];?></div>
                <div class="col-md-3 label-price-btm "><button onclick="$(this).parents('.left_ser').next('.hide_div').toggle(),start($(this),'<?=$route_name['route_name_id'];?>','<?=$route['from']['date'];?>','<?=$route['from']['route_city_id'];?>','<?=$route['to']['route_city_id'];?>')" class="btm-edit btn btn-success buy-btm"> <?=__("Choice ticket")?> <i class="fa fa-cart-arrow-down"></i></button></div>              
            </div>
            ...
            <p class="main_info"><span><?=$route_name['name'].' '.$route_name['name_i18n'];?></span><span><?=$fermann_id['name'];?></span><span><?=$bus['name_i18n'];?></span></p>
            <div class="left_ser col-md-3">
                <p><?=$city_from['city_i18n'];?></p>
                <p><?=$city_from['name_i18n'];?></p>
            </div>
            <div class="left_ser col-md-3">
                <p><?=$city_to['city_i18n'];?></p>
                <p><?=$city_to['name_i18n'];?></p>
            </div>
            <div class="left_ser col-md-3">
                <p onclick="$(this).parents('.left_ser').next('.hide_div').toggle(),start($(this),'<?=$route_name['route_name_id'];?>','<?=$route[2]['date'];?>','<?=$route[0]['route_city_id'];?>','<?=$route[1]['route_city_id'];?>')"><?=__("Information");?></p>
                <p>
                    <button class="btn badge-info"><?=$price;?></button>
                </p>
            </div>
            <div class="hide_div col-md-12 one_par">
                <div class="salon col-md-2"></div>
...
function start(thiss,RouteNameID,dateText,From,To){
    ...
    $.ajax({
            ...
            data: {route_name_id:RouteNameID,date:$('#datepicker_sell').val(),from:From,to:To,rice:1},
            async: false,
