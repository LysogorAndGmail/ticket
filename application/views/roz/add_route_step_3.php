                    ...
                    <div class="grid simple">
                        <div class="col-md-12 white-content"><!-- starp white -->                    
                            ...
                            <div class="route_nav">
                                ...
                                <a href="/roz/add_route_step_3?id=<?=$_GET['id'];?>" class="activ"><? echo __('Date and Carry');?></a>
                                ...
                                <div class="row" style="position: relative;">
                                    ...
                                    <div class="form-group">
                                        <? foreach($all_dates as $d){ 
                                            $fer_name = DB::select()->from('ferryman')->where('ferryman_id','=',$d['fer_id'])->execute()->current();?>
                                            <span class="btn btn-info btn-cons show_fer_dates" data-id="<?=$d['fer_id'];?>"><? echo $fer_name['name'];?></span>
                                            <a href="/roz/del_step_3?step_id=<?=$roz_rou['id']."&fer_id=".$d['fer_id'];?>"><i class="fa fa-times"></i></a><br />
                                        <?}?>
                                    </div>
                                    <hr />
...
$('.cl').click(function(){
    ...
    $.ajax({
        type: "POST",
        url: "/roz/save_step_3",
        data: {step_id:'<?=$roz_rou['id']?>',fer_id:FerID,d_arr:AR,bus:$('.bus').val()},
        success: function(data) {
            //alert(data);
            location="/roz/add_route_step_3?id=<?=$roz_rou['id']?>";
        },
        error:function(){
    ...
    $('.ui-datepicker-group').each(function(){
        $(this).find('tr').each(function(){
            $(this).find('a').each(function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "/roz/ajax_fer_dates",
                    data: {id:ID,day:Full,route_name_id:<?=$roz_rou['id']?>},
                    //async: false,
                    success: function(data) {
                        if(data == Full){
                            //alert(HTMLL);
