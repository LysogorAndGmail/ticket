...
<div class="panel panel-default one_people" style="position: relative;">
    <div class="panel-heading ">
        <div class="panel-title">
           <span><?=__('Place in the bus')?></span> <span class="plase label label-warning text-black"><?=$plase;?></span> |  <span class="text-complete"><?=__('Tariff')?>: <span class="info_discounts text-complete" ><? if(isset($all_discount_hidden[0])){ echo $all_discount_hidden[0]['name_simple_i18n'];}else{ echo $all_discount[0]['name_simple_i18n']; }?></span>
        </div>
        ...
        <div class="btn-group">
                ...
                <input type="hidden" class="cleent_id for_return" />
                <div class="col-md-8">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group required">
                        <label><?=__('First Name')?></label>
                        <input type="text" required="" class="soname form-control ajax_tel valid upper for_return" />
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        <label><?=__('Last Name')?></label>
                        <input type="text" class="name form-control valid upper for_return">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-3">
                    ...
                    <input type="text" class="date form-control custom_date valid tip for_return" id="date<?=$mil_sek;?>" title="format 99/99/9999" />
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                    ...
                    <input type="text" class="tel form-control valid for_return" value="<?=$sesuser[0]['phone_kod'];?>" maxlength="13"  />
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        <select class="discount form-control">
                            ...
                            <?if($ferryman_id == 68){?>
                                <option value="26">Взрослый</option>
                                <option value="14">Пенс.60лет</option>
                                <option value="15">Студ.29лет</option>
                                <option value="17">Дети 12 лет</option>
...
$('.discount').change(function(){
        ...
        $.ajax({
            ...
            error:function(code, opt, err){
                alert("Состояние : /discounts/ajax_get_discount");
            }
        });                
...
$('.clear_cleent').click(function(){
    ...
    Parent.find('.soname').attr('disabled',false);
    Parent.find('.name').val('');
    Parent.find('.name').attr('disabled',false);
    Parent.find('.tel').val('');
    Parent.find('.tel').attr('disabled',false);
    Parent.find('.date').val('');
    Parent.find('.date').attr('disabled',false);
})
...
$('.rem_plase').click(function(){
    ...
    $(this).parents('.one_par').find('.salon').find('.choise').each(function(){
        if($(this).text() == Plase){
